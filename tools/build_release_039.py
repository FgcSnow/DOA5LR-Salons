...
Changes: version 0.3.9, final documentation, new PE files signed (self-signed cert, src/tools/sign.ps1),
         installer shipped as a separate release asset (never inside the ZIP, rule since 0.3.7),
         manifest with the official GitHub release URLs + installer= lines.
No network call, no publication, no game-folder write. Output: --out folder.
...
NOTES = [
    ...
    "note=0.3.9: the installer (1.3.0) can launch the game through Steam (it opens Steam first if needed) and create a DOA5LR (Lobby Mods) desktop shortcut that opens the configuration first. Idea by Inyo, thanks!",
...
def sign(blobs: dict[str, bytes]) -> dict[str, bytes]:
    ...
    with tempfile.TemporaryDirectory(prefix="doa5lr-sign-") as tmp:
        ...
        for i, (name, blob) in enumerate(blobs.items()):
            ...
            p.write_bytes(blob); paths[name] = p
...
def readme_test(old: bytes) -> bytes:
    ...
    t = t.replace("This is an unpublished local draft.\r\nIts new executables and DLLs are unsigned.\r\n",
                  ...
                  "(it lowers antivirus false positives; it is not a security guarantee).\r\n")
...
def main() -> None:
    ...
    installer = signed.pop(INSTALLER_IN_ZIP)
    payload.update(signed)
    del payload[INSTALLER_IN_ZIP]                      # separate release asset, never in the ZIP
    ...
    for n in ("READ-ME-FIRST-EN.txt", "START-HERE.md", "InputLab/START-HERE-EN.md"):
        payload[n] = guide
    payload["InputLab/CHANGELOG-EN.md"] = CHANGES.read_bytes()
    ...
    shutil.copyfile(GUIDE, args.out / "START-HERE.md")
    ...
    # only the signature may differ from the tested preview bytes: compare sizes of the unsigned parts is not possible,
    # so record both hashes for the report
    report = {
        ...
        "signed": {n: {"tested_unsigned": tested[n], "signed": sha(installer if n == INSTALLER_IN_ZIP else payload[n])} for n in TO_SIGN},
