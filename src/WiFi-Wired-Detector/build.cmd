...
rem DOA5LR-WiFi-Wired-Detector : build release (sans journal, -DNO_LOG) + build -debug (avec journal), VERSIONINFO.
setlocal
cd /d "%~dp0"
set "BIN="
...
"%BIN%\i686-w64-mingw32-gcc.exe" -O2 -s -shared -static -Wl,--no-insert-timestamp -DNO_LOG -o DOA5LR-WiFi-Wired-Detector.asi netboost.c version.res -liphlpapi -lws2_32 || (echo BUILD release FAILED & exit /b 1)
"%BIN%\i686-w64-mingw32-gcc.exe" -O2 -s -shared -static -Wl,--no-insert-timestamp -o DOA5LR-WiFi-Wired-Detector-debug.asi netboost.c version.res -liphlpapi -lws2_32 || (echo BUILD debug FAILED & exit /b 1)
del version.res
for %%F in (DOA5LR-WiFi-Wired-Detector.asi DOA5LR-WiFi-Wired-Detector-debug.asi) do echo OK  %%F  %%~zF bytes
